...
import {
    STATUS_ADD, STATUS_CONSUME, STATUS_DAMAGE,
...
import {
    DATE_TIME_FORMAT, STATUS_ACTIVE,
    SOFT_DELETE_STATUS, ALERT_TIME_OUT_5000
} from 'app/shared';
...
export class NurseryInventoryMgntComponent implements OnInit {
    ...
    constructor(
        ...
        config.maxDate = {year: currentDate.getFullYear(), month: currentDate.getMonth() + 1,
    ...
    getSeedsInventoryList(): void {
        ...
        this.nurseryInventoryService.query({filter: { 'status.equals': STATUS_SEEDS }})
    ...
    getCoverInventoryList(): void {
        ...
        this.nurseryInventoryService.query({filter: { 'status.equals': STATUS_COVER }})
        .subscribe((res: HttpResponse<INurseryInventory[]>) => {
    ...
    getCoverFillingDetails(id): void {
        ...
        this.coverFillingDetailsService.query({
            filter: {

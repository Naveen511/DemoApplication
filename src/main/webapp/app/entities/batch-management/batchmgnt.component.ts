...
import { IBatch, BatchModel,
    ...
    STATUS_BATCH_CLOSE } from 'app/shared/model/batch.model';
...
export class BatchmgntComponent implements OnInit {
    ...
    closeBatch(batch: BatchModel): void {
        if ((this.batch.closedDate === null)
            || (this.batch.closedDate === undefined)
        ) {
    ...
    saveBatchQuantity() {
        ...
        this.batchQuantityService.create(this.batchQuantity)
        .subscribe(data => {
                // console.log(data.body);
    ...
    getParticularBatchQuantityDetails(batchId): void {
        ...
        this.batchQuantityService.query({
            filter: { 'batchId.equals': batchId }
        })
